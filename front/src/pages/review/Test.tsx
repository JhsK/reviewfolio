...
const Review = () => {
  ...
  const [isApplicant, setIsApplicant] = useState<IApplication>(null);

  const isApplicationId = useMemo(() => {
    if (currentUser.data.position === 'programmer') return isApplicant?.ProgrammerId;
    return activeApplicant?.ProgrammerId;
  }, [activeApplicant, isApplicant]);

  const { data: applicant, isSuccess } = useQuery(['applicant'], () => getApplicant(id as string), {
    onSuccess: (data) => {
      setActiveApplicant(data[0]);
      if (currentUser.data.position === 'programmer')
        setIsApplicant(data.filter((v) => currentUser.data?.programmerId === v.ProgrammerId)[0]);
    },
  });
  const { data: chatList } = useQuery(['chatList', isApplicationId], () => getComment(id as string, isApplicationId), {
    enabled: isSuccess,
  });
      ...
      if (result.isConfirmed) {
        try {
          reviewStatusMutation.mutate(
            { applicationId: isApplicant.id, programmerId: currentUser.data.programmerId },
            {
              onSuccess: () => {
    ...
    <Layout>
      ...
      {activeApplicant?.status === '리뷰 진행중' && (
        <CommentForm
          ...
          activeApplicant={isApplicant?.ProgrammerId}
        />
      )}
    </Layout>
  );
